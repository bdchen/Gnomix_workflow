...
workflow run_gnomix_from_scratch {
  input {
    Array[File] query_file
    Array[Int] chr_nr
    Boolean phase = true
    File genetic_map_file
    Array[File] reference_file
    File sample_map_file
    String output_prefix
  }

  scatter(i in range(length(query_file))) {
    call gnomix_from_scratch as gnomixs{
      input:
        query=query_file[i],
        chr=chr_nr[i],
        phase=phase,
        gene_map=genetic_map_file,
        ref_file=reference_file[i],
        samp_map=sample_map_file
  ...
  output {
    ...
    Array[File] model_file  = select_all(flatten([gnomixs.model_file]))
  }
...
task gnomix_from_scratch {
  input {

    # Gnomix from scratch inputs 
    File query
    Int chr
    Boolean phase
    File gene_map
  ...
  command <<<
    ...
    mv /cromwell_root/models/model_chr_~{chr}.pkl /cromwell_root/~{prefix}.chrom~{chr}_model.plk
  ...
  >>>
    ...
    output {
      ...
      File model_file = "~{prefix}.chrom~{chr}_model.plk"
    }
